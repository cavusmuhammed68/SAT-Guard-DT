...
UK_ATLAS_REGION_COLOURS = {
    "Highlands and Islands": "#9b6bc6",
    "North East Scotland": "#69c7df",
    "Mid Scotland and Fife": "#8fa8e8",
    "South Scotland": "#8d89c8",
    "Northern Ireland": "#69c489",
    "North East England": "#f05b93",
    "North West England": "#f5c16c",
    "Yorkshire and the Humber": "#f58b68",
    "East Midlands": "#d8578b",
    "West Midlands": "#f06d4f",
    "East of England": "#f8a693",
    "Greater London": "#e85586",
    "South East England": "#f8cf73",
    "South West England": "#e84d8c",
    "Wales": "#65bd6a",
}
...
def _uk_hex_region_colour(label: str) -> str:
    """Bloomberg-style categorical colour palette for regional DNO polygon map regions."""
    palette = {
...
def _build_uk_hex_cartogram_cells(active_region: str) -> List[Dict[str, Any]]:
    """Build a clean regional DNO polygon map similar to the supplied Bloomberg-style reference."""
...
def render_uk_atlas_style_spatial_map(region: str, places: pd.DataFrame) -> None:
    """
    Render a data-honest regional spatial intelligence map.

    The application currently contains monitored data for only two operational
    study areas: North East and Yorkshire. This figure therefore does not draw a
    selected-region cartogram and does not create synthetic Scotland, Wales, London or
    Northern Ireland cells. It draws only the selected regional DNO study area,
    using the embedded regional polygons and place-level monitored evidence.
    """
    ...
    region_label = "North East England" if region == "North East" else "Yorkshire"

    if region == "North East":
        min_lon, min_lat, max_lon, max_lat = REGIONS["North East"]["bbox"]
    else:
        min_lon, min_lat, max_lon, max_lat = REGIONS["Yorkshire"]["bbox"]

    width, height = 980, 720
    pad = 70

    def project(lon: float, lat: float) -> Tuple[float, float]:
        x = pad + (lon - min_lon) / max(max_lon - min_lon, 1e-9) * (width - 2 * pad)
        y = height - pad - (lat - min_lat) / max(max_lat - min_lat, 1e-9) * (height - 2 * pad)
        return x, y

    polygon_svg: List[str] = []
    label_svg: List[str] = []

    for feature in geojson.get("features", []):
        props = feature.get("properties", {}) or {}
        geom = feature.get("geometry", {}) or {}
        colour = props.get("colour", "#38bdf8")
        risk = safe_float(props.get("risk"))
        stroke_width = 2.4 if risk >= 65 else 1.6
        stroke = "#0f172a" if risk >= 65 else "#ffffff"
        tooltip = html.escape(
            f"{props.get('region_unit', props.get('place', 'Regional unit'))} | "
            f"nearest: {props.get('nearest_place', props.get('place', ''))} | "
            f"risk {risk:.1f}/100 | resilience {safe_float(props.get('resilience')):.1f}/100 | "
            f"failure {safe_float(props.get('failure_probability')):.1f}% | ENS {safe_float(props.get('ens')):.1f} MW"
        )
        ...
        for polygon in polygon_sets:
            ...
            if len(pts) >= 3:
                polygon_svg.append(
                    f'<polygon points="{" ".join(pts)}" fill="{colour}" stroke="{stroke}" stroke-width="{stroke_width}" opacity="0.94">'
                    f'<title>{tooltip}</title></polygon>'
                )

        cx_lon, cx_lat = _feature_centroid_lonlat(feature)
        cx, cy = project(cx_lon, cx_lat)
        unit = html.escape(str(props.get("region_unit", props.get("place", ""))))
        if unit:
            label_svg.append(f'<text x="{cx:.1f}" y="{cy:.1f}" text-anchor="middle" class="unit-label">{unit}</text>')

    marker_svg: List[str] = []
    for _, row in places.iterrows():
        x, y = project(safe_float(row.get("lon")), safe_float(row.get("lat")))
        name = html.escape(str(row.get("place", "Place")))
        postcode = html.escape(str(row.get("postcode_prefix", "")))
        risk = safe_float(row.get("final_risk_score"))
        marker_svg.append(
            f'<g class="place-marker"><circle cx="{x:.1f}" cy="{y:.1f}" r="7.5" fill="#111827" stroke="#ffffff" stroke-width="2.1">'
            f'<title>{name} ({postcode}): risk {risk:.1f}/100</title></circle>'
            f'<text x="{x+11:.1f}" y="{y+4:.1f}" class="place-label">{name}</text></g>'
        )

    svg = f"""
    <div class="regional-atlas-shell">
      <div class="regional-atlas-header">
        <div>
          <div class="regional-atlas-title">{html.escape(region_label)} spatial intelligence</div>
          <div class="regional-atlas-subtitle">Data-driven regional DNO study area only — no synthetic selected-region cells</div>
        </div>
        <div class="regional-atlas-badge">{html.escape(region)}</div>
      </div>
      <svg viewBox="0 0 {width} {height}" role="img" aria-label="{html.escape(region_label)} regional spatial intelligence map">
        <rect x="0" y="0" width="{width}" height="{height}" rx="28" fill="#ffffff"/>
        <g class="polygon-layer">{''.join(polygon_svg)}</g>
        <g class="label-layer">{''.join(label_svg)}</g>
        <g class="marker-layer">{''.join(marker_svg)}</g>
        <g class="legend-layer">
          <rect x="28" y="28" width="236" height="116" rx="18" fill="rgba(255,255,255,.92)" stroke="#e5e7eb"/>
          <circle cx="52" cy="58" r="8" fill="#111827" stroke="#fff" stroke-width="2"/>
          <text x="70" y="64" class="legend-text">Monitored city/postcode anchor</text>
          <rect x="44" y="84" width="18" height="18" rx="4" fill="#ff5a5f" stroke="#fff"/>
          <text x="70" y="99" class="legend-text">Categorical regional polygon</text>
          <line x1="44" y1="124" x2="62" y2="124" stroke="#0f172a" stroke-width="3"/>
          <text x="70" y="130" class="legend-text">Darker border = higher risk</text>
        </g>
      </svg>
      ...
      <div class="regional-note">This map intentionally shows only the selected study region because the dataset contains North East and Yorkshire locations only. Whole-UK cartograms are not used here because they would imply coverage that the model does not currently have.</div>
    </div>
    <style>
      .regional-atlas-shell {{background:#fff;border-radius:30px;padding:18px;border:1px solid #e5e7eb;box-shadow:0 24px 80px rgba(2,6,23,.18);font-family:Inter,Arial,sans-serif;}}
      .regional-atlas-header {{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:10px;}}
      .regional-atlas-title {{font-size:30px;font-weight:950;color:#111827;letter-spacing:-.04em;}}
      .regional-atlas-subtitle {{font-size:14px;color:#64748b;margin-top:4px;}}
      .regional-atlas-badge {{background:#0f172a;color:#fff;border-radius:999px;padding:10px 14px;font-size:13px;font-weight:900;}}
      .regional-atlas-shell svg {{width:100%;height:auto;display:block;}}
      .polygon-layer polygon {{transition:filter .15s, opacity .15s;}}
      .polygon-layer polygon:hover {{filter:brightness(.92);opacity:.82;}}
      .unit-label {{font-size:11px;font-weight:850;fill:#111827;paint-order:stroke;stroke:#fff;stroke-width:3px;stroke-linejoin:round;}}
      .place-label {{font-size:14px;font-weight:950;fill:#111827;paint-order:stroke;stroke:#fff;stroke-width:4px;stroke-linejoin:round;}}
      .legend-text {{font-size:13px;font-weight:750;fill:#111827;}}
      .regional-kpis {{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px;margin-top:12px;}}
      .regional-kpis div {{background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:11px 13px;}}
      .regional-kpis b {{display:block;color:#475569;font-size:12px;}}
      .regional-kpis span {{display:block;color:#0f172a;font-size:20px;font-weight:950;margin-top:3px;}}
      .regional-note {{margin-top:10px;color:#475569;font-size:13px;line-height:1.45;background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:10px 12px;}}
      @media(max-width:850px) {{.regional-kpis {{grid-template-columns:repeat(2,minmax(0,1fr));}} .regional-atlas-title {{font-size:23px;}}}}
    </style>
    """
    components.html(svg, height=860, scrolling=True)
...
def spatial_tab(
    ...
    """
    st.subheader("🌍 Spatial Intelligence — North East / Yorkshire regional risk zones")
    ...
    render_colour_legend("risk")
    render_uk_atlas_style_spatial_map(region, df)
    st.markdown("#### Local analytical layer")
    ...
    st.markdown(
        """
        <div class="note">
        <b>Spatial Intelligence update:</b> the main map now shows only the selected regional study area,
        because the available model data covers North East and Yorkshire only. The polygons are categorical
        regional units, the black markers are monitored local places, and the tables preserve the numerical
        risk, resilience, ENS and failure evidence.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
Q1_FULL_README_MARKDOWN = '\n<div class="card">\n<h2 style="color:white;margin-top:0;">SAT-Guard Digital Twin — full README</h2>\n<p style="color:#cbd5e1;line-height:1.65;">\nThis README is the in-app methodological guide. It explains every dashboard tab, the main equations, why each indicator is used, and how to interpret the direction of each score.\n</p>\n</div>\n\n## Quick interpretation guide\n\n| Variable | Good direction | Meaning |\n|---|---:|---|\n| Risk score | Lower is better | A high score means stronger operational hazard pressure. |\n| Resilience index | Higher is better | A high score means the place is more robust and recoverable. |\n| Social vulnerability | Lower is better | A high score means people and services are more sensitive to disruption. |\n| IMD / IoD vulnerability | Lower is better in this app | The app converts deprivation evidence to a 0–100 vulnerability scale, so higher means more deprived or more exposed. |\n| Failure probability | Lower is better | A high value means greater likelihood of grid-service degradation. |\n| ENS | Lower is better | Energy Not Supplied is unserved demand; higher values mean stronger technical and welfare impact. |\n| Flood depth proxy | Lower is better | Higher values indicate stronger surface-water or flood-access pressure. |\n| EV/V2G support | Higher is better | More grid-coupled EV storage can reduce ENS and outage impact. |\n| Funding priority | Higher means more urgent | It identifies places that deserve earlier engineering or resilience investment. |\n\n## Why the model uses these indicators\n\nThe model links five layers that normally sit in separate datasets: meteorology, electricity-network stress, socio-economic vulnerability, financial consequence, and intervention priority. A weather-only score would miss vulnerable communities. An outage-only score would miss incoming hazard pressure. A financial-only score would miss critical welfare impacts. The dashboard therefore keeps intermediate variables visible instead of hiding them inside a black-box model.\n\n## Tab-by-tab explanation\n\n### Executive overview\n**What it finds:** the current regional risk picture, highest-risk locations, average resilience, ENS pressure, failure probability and total financial exposure.  \n**Why it exists:** decision-makers need one page that quickly says where the stress is and whether the situation is routine, watch-level, high or severe.  \n**Main equations:**\n\n`risk_score = weather_score + pollution_score + outage_score + ENS_score + social_score + scenario_adjustment`\n\n`resilience = 100 − weighted_penalties(risk, vulnerability, grid_failure, renewable_failure, system_stress, finance)`\n\n### Simulation\n**What it finds:** dynamic operational hazard evolution under live or what-if weather.  \n**Why it exists:** risk is easier to understand when wind, rain, fronts, storm cells and city labels are animated.  \n**Main equation:**\n\n`storm_intensity = f(wind, precipitation, pressure, cloud_cover, scenario_multiplier)`\n\n### Natural hazards\n**What it finds:** hazard-specific resilience for wind storm, flood/heavy rain, drought, air-quality/heat stress and compound hazard.  \n**Why it exists:** a place can be resilient to one hazard but fragile to another.  \n**Main equation:**\n\n`hazard_resilience = base_resilience − hazard_penalty − social_penalty − outage_penalty − ENS_penalty − failure_penalty − finance_penalty`\n\n### IoD2025 socio-economic evidence\n**What it finds:** whether deprivation-domain files were matched and how social vulnerability was calculated.  \n**Why it exists:** technical outages have unequal human impact. Income, health, employment, education, housing and living-environment conditions affect recovery.  \n**Main equation:**\n\n`social_vulnerability = blend(IoD_domain_score, population_density_proxy, local_vulnerability_proxy)`\n\n### Spatial intelligence\n**What it finds:** a North East or Yorkshire regional map plus local analytical risk polygons for the selected DNO study area.  \n**Why it exists:** the app needs a clean coloured regional figure, not artificial UK-wide, Scotland, Wales, London or Northern Ireland cells.  \n**Main equations:**\n\n`regional_colour = categorical_region_palette(local_region_unit)`\n\n`local_border_intensity = risk_category(final_risk_score)`\n\n### Resilience\n**What it finds:** which locations are robust, stable, stressed or fragile.  \n**Interpretation:** high resilience is good; low resilience is bad.\n\n### Failure and investment\n**What it finds:** failure probability by hazard and ranked investment need.  \n**Main equations:**\n\n`failure_probability = 1 / (1 + exp(-z))`\n\n`z = intercept + technical_terms + weather_terms + outage_terms + cold_stress + vulnerability_terms`\n\n`investment_score = 0.30×risk + 0.22×vulnerability + 0.18×(100−resilience) + 0.13×loss_percentile + 0.10×ENS_percentile + 0.07×outage_pressure`\n\n### Scenario losses\n**What it finds:** how loss changes under extreme wind, flood, heatwave, drought, blackout stress and compound extreme events.  \n**Main equation:**\n\n`scenario_loss = live_loss × scenario_finance_multiplier × hazard_exposure_adjustment`\n\n### Finance and funding\n**What it finds:** total financial loss, VoLL loss, customer interruption loss, restoration cost, critical-service uplift and funding priority.  \n**Main equations:**\n\n`VoLL_loss = ENS_MWh × value_of_lost_load`\n\n`total_loss = VoLL_loss + customer_interruption_loss + business_disruption_loss + restoration_loss + critical_services_loss`\n\n### Monte Carlo\n**What it finds:** uncertainty ranges rather than one deterministic answer. P95 and CVaR95 show tail risk.  \n**Main equations:**\n\n`storm_shock ~ Normal(0, 1)`\n\n`wind = base_wind × exp(0.16×storm_shock + noise)`\n\n`loss = VoLL + restoration_lognormal_tail + social_uplift`\n\n### Validation / black-box review\n**What it finds:** sanity checks for monotonicity, transparency and evidence availability.  \n**Why it exists:** the model is intended to be transparent and auditable, not a hidden AI classifier.\n\n### Data / Export\n**What it finds:** downloadable tables for place-level risk, postcode recommendation, Monte Carlo and funding evidence.\n\n## Direction of important concepts\n\n**Resilience:** high is good. A score of 80 means robust; a score of 35 means stressed or fragile.  \n**Vulnerability:** high is bad. It means greater human sensitivity to the same technical event.  \n**IMD/IoD:** in raw government data, rank direction can vary by representation. In this dashboard it is converted so high always means more deprivation or higher vulnerability.  \n**Risk:** high is bad. It means stronger stress and a greater need for monitoring or intervention.  \n**ENS:** high is bad because it means customers or services are not being supplied.  \n**EV/V2G:** high support is good because it can offset ENS and support local recovery.\n\n## Model limitations\n\nThis is a research prototype for planning, teaching and decision-support exploration. It is not a replacement for DNO operational control, protection studies, asset-condition inspections or formally calibrated reliability analysis. Before policy or engineering use, the assumptions should be calibrated with observed outages, customer interruptions, CML/CI records, feeder topology, substation exposure, restoration duration and local asset data.\n\n## Extended calibration and glossary appendix\n\n### Calibration note 1: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 2: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 3: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 4: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 5: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 6: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 7: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 8: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 9: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 10: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 11: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 12: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 13: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 14: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 15: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 16: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 17: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 18: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 19: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 20: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 21: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 22: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 23: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 24: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 25: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 26: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 27: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 28: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 29: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 30: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 31: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 32: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 33: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 34: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 35: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 36: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 37: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 38: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 39: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 40: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 41: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 42: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 43: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 44: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 45: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 46: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 47: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 48: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 49: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 50: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 51: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 52: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 53: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 54: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 55: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 56: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 57: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 58: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 59: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 60: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 61: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 62: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 63: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 64: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 65: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 66: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 67: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 68: IMD / IoD score\n\n**Interpretation:** In this app higher means more deprived / more vulnerable.\n\n**Why it is used:** Raw deprivation ranks can be confusing because some official ranks use rank 1 as most deprived. The app converts evidence to a 0 to 100 vulnerability direction so interpretation is consistent.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 69: Failure probability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It is a calibrated probability-like indicator of grid failure stress. Cold weather, wind, rain, outage clustering, ENS and vulnerability all raise the result.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 70: ENS\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** Energy Not Supplied estimates unserved demand. It is one of the strongest indicators of technical and financial severity.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 71: Flood depth proxy\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It converts rainfall, terrain/fallback exposure and local stress into an indicative water-depth pressure proxy for spatial comparison.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 72: EV/V2G support\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It estimates how electric vehicles could provide distributed emergency storage and reduce ENS during stress events.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 73: Risk score\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It combines weather hazard, outage pressure, air-quality stress, ENS and social exposure to identify where the grid is more likely to experience disruption.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 74: Resilience index\n\n**Interpretation:** Higher is better.\n\n**Why it is used:** It expresses the remaining capacity of a place to absorb, withstand and recover from stress after risk, vulnerability, failure and financial pressure are considered.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n\n### Calibration note 75: Social vulnerability\n\n**Interpretation:** Higher is worse.\n\n**Why it is used:** It represents human sensitivity to disruption. A high value means the same technical outage is likely to cause stronger welfare and service impacts.\n\n**Practical reading:** compare the value with neighbouring places, then check the evidence columns before making an engineering or funding judgement.\n'


def render_readme_tab() -> None:
    st.subheader("README — model, equations, tab logic and interpretation")
    st.markdown(Q1_FULL_README_MARKDOWN, unsafe_allow_html=True)
